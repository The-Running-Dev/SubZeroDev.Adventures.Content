...
export const WHAT_WOULD_LUCIFER_DO_ENGINEERS_CUT_CAMPAIGN_ID =
...
export const whatWouldLuciferDoEngineersCutCatalog: PortableCatalog = {
  ...
  duration: "25–35 min",
  contentNotice:
    "Strong language, religious satire, recognizable parody, and technical jargon used completely unironically.",
  featured: false,
  hidden: true,
};
...
prediction({
  ...
  correctLabel:
    "A 95, docked for 'not enough comments' — followed later by an award for exceptional student",
  correctEffects: [
  ...
  extraChoices: [
    opt(
      "ch3_p1",
      "leave",
      "Close the laptop. You're not doing this.",
      "ending_walk_away",
    ),
  ],
});
...
prediction({
  ...
  correctLabel:
    "It triggers a full architecture review, escalating to the division head",
  correctEffects: [
    inc("predictions_correct"),
    inc("streak"),
    inc("bureaucratic_escalation"),
  ...
  wrongOptions: [
    ...
    {
      suffix: "quiet",
      label: "Nothing — a quiet merge, nobody notices for months",
      effects: [put("streak", 0), inc("reasonable_assumption")],
    },
    {
      suffix: "fired",
      label: "He gets fired for touching infrastructure without a ticket",
      effects: [put("streak", 0)],
    },
  ],
  missReveal:
...
pick(
  ...
  [
    ...
    opt(
      "disc_hub_ch3",
      "statemachine",
      "The file system state machine (2005–2010)",
      "disc_statemachine",
...
prediction({
  ...
  setup:
    ...
    "What Would Lucifer Do?",
  correctLabel:
    "Build the WinGet package first, submit it, then apply for the job",
  correctEffects: [
    inc("predictions_correct"),
    inc("streak"),
  ...
  wrongOptions: [
    {
      suffix: "apply",
      label: "Apply normally, mention the missing package in the cover letter",
      effects: [put("streak", 0), inc("reasonable_assumption")],
    },
    {
      suffix: "email",
      label: "Email the company suggesting they build one themselves",
      effects: [put("streak", 0)],
    },
  ],
  missReveal:
...
prediction({
  ...
  wrongOptions: [
    {
      suffix: "ignore",
      label: "Ignore the banner — it was probably a joke",
      effects: [put("streak", 0), inc("reasonable_assumption")],
    },
    {
      suffix: "screenshot",
      label: "Screenshot it for later, keep reading the source",
      effects: [put("streak", 0)],
    },
  ],
  missReveal:
...
prediction({
  ...
  wrongOptions: [
    {
      suffix: "rewrite",
      label: "Immediately rewrite everything from scratch",
      effects: [put("streak", 0)],
    },
    {
      suffix: "human",
      label: "Escalate to an actual human architect",
      effects: [put("streak", 0), inc("reasonable_assumption")],
    },
    {
      suffix: "rust",
      label: "Demand a full rewrite in Rust and reconvene",
      effects: [put("streak", 0)],
    },
  ],
  missReveal:
...
prediction({
  ...
  correctLabel:
    "'Tomorrow someone will use the top-tier model, high effort, to alphabetize a JSON file.'",
  correctEffects: [
    inc("predictions_correct"),
    inc("streak"),
...
prediction({
  ...
  wrongOptions: [
    ...
    {
      suffix: "script",
      label: "Write a one-line shell alias and move on with his life",
      effects: [put("streak", 0)],
    },
  ],
  missReveal:
...
prediction({
  ...
  correctLabel:
    "50 of 51 work units complete — about 98%, across three entirely different game genres",
  correctEffects: [
    inc("predictions_correct"),
    inc("streak"),
    inc("ridiculous_engineering"),
  ...
  wrongOptions: [
    ...
    },
    {
      suffix: "sentient",
      label:
        "Fully complete, sentient, and filing its own pull requests — reality couldn't possibly beat that",
...
prediction({
  ...
  correctLabel:
    "Build a full blogging platform, with an API and an MCP server, to write about the engine",
  correctEffects: [
    inc("predictions_correct"),
    inc("streak"),
    inc("unnecessary_escalation"),
  ...
  next: "ch8_p8",
  extraChoices: [
    opt("ch8_p4", "read_roadmap", "(Read the roadmap first)", "disc_roadmap", {
...
pick(
  ...
  [
    opt("verdict_hub", "novice", "Read your verdict", "ending_tier_novice", {
      showWhen: between("predictions_correct", 0, 3),
    }),
    opt(
      "verdict_hub",
      "apprentice",
      ...
      { showWhen: between("predictions_correct", 4, 7) },
    ),
    opt("verdict_hub", "fluent", "Read your verdict", "ending_tier_fluent", {
      showWhen: between("predictions_correct", 8, 11),
    }),
    opt(
      "verdict_hub",
      "disturbing",
      ...
      { showWhen: between("predictions_correct", 12, 14) },
    ),
    opt(
      "verdict_hub",
      "transcendent",
      ...
      { showWhen: atLeast("predictions_correct", 15) },
    ),
    opt(
      "verdict_hub",
      "no_fucking_way",
...
const ENDING_ACHIEVEMENTS: readonly (readonly [
  ...
  [
    ...
    "no_fucking_way",
    "There Is No Fucking Way",
    "Correctly predict every exceptionally improbable event in the campaign.",
  ],
  [
    "reasonable_human_being",
    "reasonable_human_being",
    "Reasonable Human Being",
    "Repeatedly assume Lucifer will behave like a normal engineer.",
  ],
];
...
const achievements: AchievementDefinitionSource[] = [
  ...
  achievement(
    ...
    atLeast("predictions_correct", 13),
  ),
  achievement(
  ...
  achievement(
    "documented_menace",
    "Documented Menace",
    "Correctly predict every one of the genuinely unpredictable decisions.",
    atLeast("no_fucking_way", 3),
  ),
  achievement(
  ...
  achievement(
    ...
    "Find the Perl/CGI website wizard, years ahead of its time.",
    visited("disc_wizard"),
  ),
  achievement(
    "read_the_full_policy",
    "Reasoning Should Scale",
    "Read the full AI Model Selection Policy, not just the summary.",
...
const TITLE: AuthoredText = {
  key: "wwldx.campaign.title",
  text: "What Would Lucifer Do? — Engineer's Cut",
};
