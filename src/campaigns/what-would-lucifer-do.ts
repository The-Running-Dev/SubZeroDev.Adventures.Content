...
import type { Consequence } from "@the-running-dev/game-engine/authoring";
...
export const whatWouldLuciferDoMigration: PortableMigration = {
  fromVersion: "1.0.0",
};

// ---------------------------------------------------------------------------
// Authoring helpers — shared with what-would-lucifer-do-engineers-cut.ts and
// saki-quest-for-redemption.ts via _shared/story-graph-helpers.ts
// ---------------------------------------------------------------------------
...
function prediction(spec: PredictionSpec): void {
  ...
  const choices: ChoiceSource[] = [
    opt(spec.id, `${spec.id}_correct`, spec.correctLabel, hitId, {
      effects: spec.correctEffects,
    }),
    ...spec.wrongOptions.map((w) =>
      opt(
        ...
        `${spec.id}_${w.suffix}`,
        w.label,
        w.gotoSuffix !== undefined ? `${spec.id}_${w.gotoSuffix}` : missId,
        {
          effects: w.effects,
        },
      ),
  ...
  nodes[spec.id] = {
    kind: "choice",
    text: text(spec.id, "text", spec.setup),
    choices,
  };
...
pick(
  ...
  [
    opt(
      "ch1_scene1",
      "argue",
      "Get out and argue back — match the energy",
      "ch1_p1_miss",
      {
    ...
    opt(
      "ch1_scene1",
      "laugh",
      "Start laughing and let the man yell at nothing",
      "ch1_p1_hit",
      {
        effects: [
          inc("predictions_correct"),
          inc("streak"),
          inc("surprisingly_reasonable"),
        ],
      },
    ),
    opt(
      "ch1_scene1",
      "police",
      "Call the police to report the blocked lane",
      "ch1_p1_miss",
      {
    ...
    opt(
      "ch1_scene1",
      "leave",
      "Close the laptop. You did not sign up to predict a man's driving choices.",
      "ending_walk_away",
    ),
  ],
);
...
pick(
  ...
  [
    opt(
      "ch1_scene2",
      "nudge",
      "Nudge past, horn for horn, make the point back",
      "ch1_p2_miss",
      {
        effects: [put("streak", 0)],
      },
    ...
    opt(
      "ch1_scene2",
      "reverse",
      "Reverse out and take the long way around the block",
      "ch1_p2_miss",
      {
        effects: [put("streak", 0), inc("reasonable_assumption")],
      },
    ...
    opt(
      "ch1_scene2",
      "thumbsup",
      "Drive around both cars, still laughing, thumbs-up on the way past",
      "ch1_p2_hit",
      {
        effects: [
          inc("predictions_correct"),
...
fork(
  "ch1_p2_fork",
  "He's still not sure what the expected response was supposed to be.",
  [
...
prediction({
  ...
  wrongOptions: [
    {
      suffix: "lift",
      label: "Lift off the throttle and let the car settle",
      effects: [put("streak", 0), inc("reasonable_assumption")],
    },
    {
      suffix: "brake",
      label: "Brake hard and hope for the best",
      effects: [put("streak", 0)],
    },
    {
      suffix: "pray",
      label:
        "Grip the wheel, say nothing, negotiate silently with the guardrail",
      effects: [put("streak", 0)],
    },
  ],
  missReveal:
...
prediction({
  ...
  wrongOptions: [
    {
      suffix: "slow",
      label: "Slow down for the rest of the drive — the point has been proven",
      effects: [put("streak", 0), inc("reasonable_assumption")],
    },
    {
      suffix: "stop",
      label: "Pull over and take a minute to consider recent life choices",
      effects: [put("streak", 0)],
    },
  ],
  missReveal:
...
prediction({
  ...
  correctLabel:
    "A 95, docked for 'not enough margin notes explaining his own work' — followed later by an award for exceptional student",
  correctEffects: [
    inc("predictions_correct"),
    inc("streak"),
    inc("bureaucratic_escalation"),
  ],
  hitReveal:
    "Correct.\n\n" +
  ...
  wrongOptions: [
    {
      suffix: "perfect",
      label: "A perfect score and public praise",
      effects: [put("streak", 0), inc("reasonable_assumption")],
    },
    {
      suffix: "fail",
      label: "Failed outright for not following the assignment format",
      effects: [put("streak", 0)],
    },
    {
      suffix: "ignored",
      label: "A shrug, a passing grade, nobody reads a hundred pages",
      effects: [put("streak", 0)],
    },
  ],
  missReveal:
...
prediction({
  ...
  correctLabel:
    "It triggers a full process review, escalating to the division head",
  correctEffects: [
    inc("predictions_correct"),
    inc("streak"),
    inc("bureaucratic_escalation"),
  ...
  wrongOptions: [
    {
      suffix: "thanks",
      label: "A company-wide thank-you and a raise",
      effects: [put("streak", 0)],
    },
    {
      suffix: "quiet",
      label: "Nothing — a quiet rollout, nobody notices for months",
      effects: [put("streak", 0), inc("reasonable_assumption")],
    },
    {
      suffix: "fired",
      label: "He gets fired for touching procedure without a ticket",
      effects: [put("streak", 0)],
    },
  ],
  missReveal:
...
prediction({
  ...
  setup:
    ...
    "What Would Lucifer Do?",
  correctLabel:
    "Nothing. Let the pattern repeat; the signal doesn't change to match the doubt.",
  correctEffects: [
    inc("predictions_correct"),
    inc("streak"),
    inc("surprisingly_reasonable"),
  ...
  wrongOptions: [
    {
      suffix: "argue",
      label: "Argue the point line by line with supporting documentation",
      effects: [put("streak", 0)],
    },
    {
      suffix: "shrink",
      label: "Quietly shrink the CV to sound more believable",
      effects: [put("streak", 0), inc("reasonable_assumption")],
    },
  ],
  missReveal:
...
prediction({
  ...
  setup:
    ...
    "What Would Lucifer Do?",
  correctLabel:
    "Write and submit the missing directory listing first, then apply for the job",
  correctEffects: [
    inc("predictions_correct"),
    inc("streak"),
    inc("ridiculous_engineering"),
  ],
  hitReveal:
    "Correct.\n\n" +
  ...
  wrongOptions: [
    {
      suffix: "apply",
      label: "Apply normally, mention the missing listing in the cover letter",
      effects: [put("streak", 0), inc("reasonable_assumption")],
    },
    {
      suffix: "email",
      label: "Email the company suggesting they list themselves",
      effects: [put("streak", 0)],
    },
  ],
  missReveal:
...
prediction({
  ...
  wrongOptions: [
    {
      suffix: "accept",
      label: "Accept immediately — a job is a job",
      effects: [put("streak", 0), inc("reasonable_assumption")],
      gotoSuffix: "decline_flag",
    },
    {
      suffix: "negotiate",
      label: "Counter-offer for more, stay in the conversation",
      effects: [put("streak", 0)],
    },
  ],
  missReveal:
...
prediction({
  ...
  correctEffects: [
    ...
    inc("streak"),
    inc("ridiculous_engineering"),
    put("applied_after_decline", true),
  ],
  hitReveal:
    "Correct.\n\n" +
  ...
  wrongOptions: [
    {
      suffix: "ignore",
      label: "Ignore the line — it was probably a joke",
      effects: [put("streak", 0), inc("reasonable_assumption")],
    },
    {
      suffix: "screenshot",
      label: "Screenshot it for later, keep reading the fine print",
      effects: [put("streak", 0)],
    },
  ],
  missReveal:
...
prediction({
  ...
  wrongOptions: [
    {
      suffix: "unplug",
      label: "Close the session and walk away without another word",
      effects: [put("streak", 0), inc("reasonable_assumption")],
    },
    {
      suffix: "complain",
      label: "File a calm, detailed bug report about the behavior",
      effects: [put("streak", 0)],
    },
  ],
  missReveal:
...
prediction({
  ...
  wrongOptions: [
    {
      suffix: "rewrite",
      label: "Immediately start over from scratch",
      effects: [put("streak", 0)],
    },
    {
      suffix: "human",
      label: "Escalate to an actual senior manager",
      effects: [put("streak", 0), inc("reasonable_assumption")],
    },
    {
      suffix: "rust",
      label: "Demand a completely different approach and reconvene",
      effects: [put("streak", 0)],
    },
  ],
  missReveal:
...
prediction({
  ...
  correctLabel:
    "'Tomorrow someone will pull in the most expensive specialist in the building, at full rate, to alphabetize a list of names.'",
  correctEffects: [
    inc("predictions_correct"),
    inc("streak"),
    inc("unnecessary_escalation"),
  ],
  hitReveal:
    "Correct.\n\n" +
  ...
  wrongOptions: [
    {
      suffix: "optimistic",
      label: "'Eventually, once people see the savings.'",
      effects: [put("streak", 0), inc("reasonable_assumption")],
    },
    {
      suffix: "give_up",
      label: "'No. Nobody reads policies.'",
      effects: [put("streak", 0)],
    },
  ],
  missReveal:
...
prediction({
  ...
  correctLabel:
    "Build a direct pipeline of his own, so he never has to log into that website again",
  correctEffects: [
    inc("predictions_correct"),
    inc("streak"),
    inc("ridiculous_engineering"),
  ...
  wrongOptions: [
    {
      suffix: "tolerate",
      label: "Tolerate the thirty-seven seconds like everyone else does",
      effects: [put("streak", 0), inc("reasonable_assumption")],
    },
    {
      suffix: "script",
      label: "Write a one-line shortcut command and move on with his life",
      effects: [put("streak", 0)],
    },
  ],
  missReveal:
...
prediction({
  ...
  correctLabel:
    "A way for an AI assistant to just say 'publish this' and have it happen",
  correctEffects: [
    inc("predictions_correct"),
    inc("streak"),
    inc("unnecessary_escalation"),
  ...
  wrongOptions: [
    {
      suffix: "stop",
      label: "Nothing — the pipeline is already good enough",
      effects: [put("streak", 0), inc("reasonable_assumption")],
    },
    {
      suffix: "sell",
      label: "Package it up and sell it as a product",
      effects: [put("streak", 0)],
    },
  ],
  missReveal:
...
prediction({
  ...
  correctLabel:
    "An article whose headline is that Lucifer improved the automation responsible for writing the article",
  correctEffects: [
    inc("predictions_correct"),
    inc("streak"),
    inc("philosophical_detours"),
  ],
  hitReveal:
    "Correct.\n\n" +
  ...
  wrongOptions: [
    {
      suffix: "loop",
      label: "An infinite loop that crashes the whole pipeline",
      effects: [put("streak", 0)],
    },
    {
      suffix: "nothing",
      label: "Nothing unusual — it just quietly keeps working",
      effects: [put("streak", 0), inc("reasonable_assumption")],
    },
  ],
  missReveal:
...
prediction({
  ...
  setup:
    ...
    "What Would Lucifer Do?",
  correctLabel:
    "Treat it as cosmic theater — laugh, and take the joke straight to God",
  correctEffects: [
    inc("predictions_correct"),
    inc("streak"),
    inc("philosophical_detours"),
  ...
  wrongOptions: [
    {
      suffix: "offended",
      label: "Feel mildly insulted that permission was ever in question",
      effects: [put("streak", 0)],
    },
    {
      suffix: "grateful",
      label: "Write back with sincere thanks for the generosity",
      effects: [put("streak", 0), inc("reasonable_assumption")],
    },
  ],
  missReveal:
...
prediction({
  ...
  correctLabel:
    "Whether he could email gravity and grant it permission to keep working",
  correctEffects: [
    inc("predictions_correct"),
    inc("streak"),
    inc("philosophical_detours"),
  ...
  wrongOptions: [
    {
      suffix: "ask_visa",
      label: "Whether the same trick would work on his visa paperwork",
      effects: [put("streak", 0), inc("reasonable_assumption")],
    },
    {
      suffix: "nothing",
      label: "Nothing — he lets the moment pass without a follow-up",
      effects: [put("streak", 0)],
    },
  ],
  missReveal:
...
prediction({
  ...
  setup:
    ...
    "What Would Lucifer Do?",
  correctLabel:
    "Kill it instantly, and describe it as enforcing the consequences already outlined in the agreement",
  correctEffects: [
    inc("predictions_correct"),
    inc("streak"),
    inc("no_fucking_way"),
    put("met_the_fly", true),
  ],
  hitReveal:
    "Correct.\n\n" +
  ...
  wrongOptions: [
    {
      suffix: "mercy",
      label: "Let it go one more time — patience over principle",
      effects: [put("streak", 0), inc("reasonable_assumption")],
    },
    {
      suffix: "capture",
      label: "Trap it under a glass and release it outside, ever the diplomat",
      effects: [put("streak", 0)],
    },
  ],
  missReveal:
...
prediction({
  ...
  setup:
    ...
    "What Would Lucifer Do?",
  correctLabel:
    "Ask why he'd write those mechanics for just one game, and start building an engine instead",
  correctEffects: [
    inc("predictions_correct"),
    inc("streak"),
    inc("ridiculous_engineering"),
  ...
  wrongOptions: [
    {
      suffix: "close",
      label:
        "Close the laptop and go to bed — it was a fair question, nothing more",
      effects: [put("streak", 0), inc("reasonable_assumption")],
    },
    {
      suffix: "playnormal",
      label: "Just replay the original game like a normal person",
      effects: [put("streak", 0)],
    },
  ],
  missReveal:
...
prediction({
  ...
  correctLabel:
    "50 of 51 items on the list are done — about 98%, across three entirely different game genres",
  correctEffects: [
    inc("predictions_correct"),
    inc("streak"),
    inc("ridiculous_engineering"),
  ...
  wrongOptions: [
    {
      suffix: "prototype",
      label: "A rough prototype, maybe 40% of the way there",
      effects: [put("streak", 0), inc("reasonable_assumption")],
    },
    {
      suffix: "abandoned",
      label: "Abandoned within a week for something else entirely",
      effects: [put("streak", 0)],
    },
    {
      suffix: "sentient",
      label:
        "Fully complete, sentient, and filing its own paperwork — reality couldn't possibly beat that",
...
prediction({
  ...
  correctLabel:
    "Build a full blogging platform, with a publishing pipeline of its own, to write about the engine",
  correctEffects: [
    inc("predictions_correct"),
    inc("streak"),
    inc("unnecessary_escalation"),
  ...
  wrongOptions: [
    {
      suffix: "console",
      label: "A small game console, for testing",
      effects: [put("streak", 0)],
    },
    {
      suffix: "nothing",
      label: "Nothing — the engine was the only thing he built that stretch",
      effects: [put("streak", 0), inc("reasonable_assumption")],
    },
  ],
  missReveal:
...
prediction({
  ...
  correctLabel:
    "'I used to think it was punishment. Now I think it's customer support.'",
  correctEffects: [
    inc("predictions_correct"),
    inc("streak"),
    inc("philosophical_detours"),
  ...
  wrongOptions: [
    {
      suffix: "vacation",
      label: "'Honestly? It's kind of a vacation.'",
      effects: [put("streak", 0)],
    },
    {
      suffix: "test",
      label: "'A test. It's always been a test.'",
      effects: [put("streak", 0), inc("reasonable_assumption")],
    },
  ],
  missReveal:
...
prediction({
  ...
  correctLabel:
    "Neo and Agent Smith are the same thing from different perspectives — both anomalies",
  correctEffects: [
    inc("predictions_correct"),
    inc("streak"),
    inc("philosophical_detours"),
  ...
  wrongOptions: [
    {
      suffix: "villain",
      label: "Neo is the hero and Smith is just a villain, full stop",
      effects: [put("streak", 0), inc("reasonable_assumption")],
    },
    {
      suffix: "irrelevant",
      label: "Neither one is relevant to the actual argument",
      effects: [put("streak", 0)],
    },
  ],
  missReveal:
...
prediction({
  ...
  correctLabel:
    "Treat it as obviously inevitable — of course this exists, look at everything that led here",
  correctEffects: [
    inc("predictions_correct"),
    inc("streak"),
    inc("no_fucking_way"),
  ...
  wrongOptions: [
    {
      suffix: "never",
      label: "Insist nobody would ever actually build this",
      effects: [put("streak", 0)],
    },
    {
      suffix: "funding",
      label: "Assume it must have required outside funding and a pitch deck",
      effects: [put("streak", 0)],
    },
    {
      suffix: "joke",
      label: "Dismiss it as a one-off joke, unlikely to go further",
      effects: [put("streak", 0), inc("reasonable_assumption")],
    },
  ],
  missReveal:
...
pick(
  ...
  [
    opt(
      "disc_hub_ch3",
      "website",
      "The suspicious website (1997–2005)",
      "disc_website",
      { showWhen: unvisited("disc_website") },
    ),
    opt(
      "disc_hub_ch3",
      "wizard",
      "The wizard that shouldn't exist yet (2000–2001)",
      "disc_wizard",
      { showWhen: unvisited("disc_wizard") },
    ),
    opt(
      "disc_hub_ch3",
      "statemachine",
      "The self-organizing filing system (2005–2010)",
      "disc_statemachine",
      { showWhen: unvisited("disc_statemachine") },
    ),
    opt(
      "disc_hub_ch3",
      "webapi",
      "The 'you can do that?' moment (2018)",
      "disc_webapi",
      { showWhen: unvisited("disc_webapi") },
    ),
  ...
  nodes["ch5_p3"] = {
    ...
    choices: [
      ...
      opt(
        "ch5_p3",
        "read_policy",
        "(Read the full policy first)",
        "disc_model_policy",
        { showWhen: unvisited("disc_model_policy") },
      ),
    ],
  };
}
  ...
  nodes["ch6_p3"] = {
    ...
    choices: [
      ...
      opt(
        "ch6_p3",
        "read_pipeline",
        "(Look at the before-and-after pipeline first)",
        "disc_pipeline",
        { showWhen: unvisited("disc_pipeline") },
      ),
    ],
  };
}
  ...
  nodes["ch8_p2"] = {
    ...
    choices: [
      ...
      opt(
        "ch8_p2",
        "read_episode",
        "(Read how Episode One got filed first)",
        "disc_episode_one",
        { showWhen: unvisited("disc_episode_one") },
      ),
    ],
  };
}
  ...
  nodes["ch8_p4"] = {
    ...
    choices: [
      ...
      opt(
        ...
        "disc_roadmap",
        { showWhen: unvisited("disc_roadmap") },
      ),
    ],
  };
}
...
pick(
  ...
  "Twenty-six predictions. No more incidents. Only the tally left.\n\n" +
    ...
    "make sense.",
  [
    opt("verdict_hub", "novice", "Read your verdict", "ending_tier_novice", {
      showWhen: between("predictions_correct", 0, 5),
    }),
    opt(
      "verdict_hub",
      "apprentice",
      "Read your verdict",
      "ending_tier_apprentice",
      { showWhen: between("predictions_correct", 6, 11) },
    ),
    opt("verdict_hub", "fluent", "Read your verdict", "ending_tier_fluent", {
      showWhen: between("predictions_correct", 12, 17),
    }),
    opt(
      "verdict_hub",
      "disturbing",
      "Read your verdict",
      "ending_tier_disturbing",
      { showWhen: between("predictions_correct", 18, 22) },
    ),
    opt(
      "verdict_hub",
      "transcendent",
      "Read your verdict",
      "ending_tier_transcendent",
      { showWhen: atLeast("predictions_correct", 23) },
    ),
    opt(
      "verdict_hub",
      "no_fucking_way",
      "Read the special verdict",
      "ending_no_fucking_way",
      {
...
const ENDING_ACHIEVEMENTS: readonly (readonly [
  string,
  string,
  string,
  string,
  ...
  [
    ...
    "walk_away",
    "You Closed The Laptop",
    "Decline to make a single prediction.",
  ],
  [
    "tier_novice",
    "tier_novice",
    "This Is Probably Healthy",
    "Finish with a genuinely low accuracy.",
  ],
  [
    "tier_apprentice",
    "tier_apprentice",
    "Monitor This Condition",
    "Finish with a modest, hard-won accuracy.",
  ],
  [
    "tier_fluent",
    "tier_fluent",
    "Ahead Of The Rubric",
    "Finish with just over half of all predictions correct.",
  ],
  [
    "tier_disturbing",
    "tier_disturbing",
    "Please Explain Yourself",
    "Predict nearly everything correctly.",
  ],
  [
    "tier_transcendent",
    "tier_transcendent",
    "How Did You Obtain This Information",
    "Finish with a near-perfect accuracy.",
  ],
  [
    "no_fucking_way",
    "no_fucking_way",
    "There Is No Fucking Way",
    "Correctly predict an unbroken run of exceptionally improbable events.",
  ],
  [
    "reasonable_human_being",
    "reasonable_human_being",
    "Reasonable Human Being",
    "Repeatedly assume Lucifer will behave like a normal person.",
  ],
];
...
const achievements: AchievementDefinitionSource[] = [
  ...
  achievement(
    "pattern_recognition",
    "Pattern Recognition",
    "Correctly predict four consecutive decisions.",
    atLeast("streak", 4),
  ),
  achievement(
    "unfortunately_you_get_him",
    "Unfortunately, You Get Him",
    "Reach a high prediction accuracy.",
    atLeast("predictions_correct", 20),
  ),
  achievement(
  ...
  achievement(
    "met_the_fly",
    "A Beautiful Treaty",
    "Witness the negotiation that failed spectacularly.",
    flag("met_the_fly", true),
  ),
  achievement(
    "named_the_ai",
    "Jar-Jar2R2",
    "Witness the machine get renamed after two idiot Star Wars characters.",
    flag("named_the_ai", true),
  ),
  achievement(
    "declined_the_money",
    "A 2004 Salary",
    ...
    {
      all: [
        flag("declined_the_money", true),
        flag("applied_after_decline", true),
      ],
    },
  ),
  achievement(
    "documented_menace",
    "Documented Menace",
    "Correctly predict six or more of the genuinely unpredictable decisions.",
    atLeast("no_fucking_way", 6),
  ),
  achievement(
    "state_of_the_art_paperwork",
    "State Of The Art Paperwork",
    "Correctly predict both authored bureaucratic escalations.",
    atLeast("bureaucratic_escalation", 2),
  ),
  achievement(
    "brought_alan_watts_into_this",
    "You Brought Alan Watts Into This",
    "Correctly predict five or more philosophical detours.",
    atLeast("philosophical_detours", 5),
  ),
  achievement(
    "infrastructure_for_a_feeling",
    "Infrastructure For A Feeling",
    "Correctly predict every instance of ridiculous engineering.",
    atLeast("ridiculous_engineering", 5),
  ),
  achievement(
    "escalation_as_a_service",
    "Escalation As A Service",
    "Correctly predict every instance of purely unnecessary escalation.",
    atLeast("unnecessary_escalation", 3),
  ),
  achievement(
    "wizard_that_shouldnt_exist",
    "The Wizard That Shouldn't Exist Yet",
    "Find the website-building wizard, years ahead of its time.",
    visited("disc_wizard"),
  ),
  achievement(
    "read_the_full_policy",
    "Reasoning Should Scale",
    "Read the full helper-selection policy, not just the summary.",
    visited("disc_model_policy"),
  ),
  achievement(
    "saw_the_thumbs_up",
    "Universal Gesture",
    "Witness the thumbs-up at the end of the traffic incident.",
    flag("saw_the_thumbs_up", true),
  ),
];
...
const TITLE: AuthoredText = {
  key: "wwld.campaign.title",
  text: "What Would Lucifer Do?",
};
  ...
  variables: {
    ...
    no_fucking_way: {
      ...
      label: text(
        "var_no_fucking_way",
        "label",
        "There Is No Fucking Way Events",
      ),
    },
    bureaucratic_escalation: {
      ...
      visible: true,
      label: text(
        "var_bureaucratic_escalation",
        "label",
        "Bureaucratic Escalation Predicted",
      ),
    },
    unnecessary_escalation: {
      ...
      visible: true,
      label: text(
        "var_unnecessary_escalation",
        "label",
        "Unnecessary Escalation Predicted",
      ),
    },
    surprisingly_reasonable: {
      ...
      visible: true,
      label: text(
        "var_surprisingly_reasonable",
        "label",
        "Surprisingly Reasonable Behavior Predicted",
      ),
    },
    philosophical_detours: {
      ...
      visible: true,
      label: text(
        "var_philosophical_detours",
        "label",
        "Philosophical Detours Anticipated",
      ),
    },
    ridiculous_engineering: {
      ...
      visible: true,
      label: text(
        "var_ridiculous_engineering",
        "label",
        "Ridiculous Engineering Solutions Anticipated",
      ),
    },
    reasonable_assumption: {
      ...
      visible: true,
      label: text(
        "var_reasonable_assumption",
        "label",
        "Reasonable Assumptions Made (Incorrectly)",
      ),
    },
...
export function buildWhatWouldLuciferDoCampaign(
  ...
  const campaign: Campaign = {
    ...
    migrateState: (state, fromVersion) =>
      migrateV1AdventureState(state, fromVersion, source, {}),
  };
