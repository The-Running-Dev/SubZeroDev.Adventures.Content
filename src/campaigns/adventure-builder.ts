...
import type { StoryGraphKindState } from "@the-running-dev/game-engine/authoring";
import {
...
export interface AdventureConfig {
  ...
  readonly startAliases?: readonly {
    readonly id: string;
    readonly label: string;
...
function oneChoice(
  ...
  return {
    ...
    choices: [
      {
        id: `${id}_proceed`,
        label: authored(namespace, id, "proceed", label),
        goto,
      },
    ],
  };
...
export function createAdventureSource(
  ...
  const nodes: StoryGraphCampaignSource["nodes"] = Object.create(
    null,
  ) as StoryGraphCampaignSource["nodes"];
  ...
  for (const route of config.routes) {
    ...
    nodes[`${prefix}_event_1`] = {
      ...
      text: authored(
        ...
        `${prefix}_event_1`,
        "text",
        "The day contributes its own amendment.",
      ),
      transitions: [
        {
          weight: 1,
          effects: [{ op: "increment", var: "preparation", by: 1 }],
          goto: `${prefix}_event_1a`,
        },
        {
          weight: 1,
          effects: [{ op: "increment", var: "pressure", by: 1 }],
          goto: `${prefix}_event_1b`,
        },
      ],
    };
    nodes[`${prefix}_event_1a`] = oneChoice(
      config.namespace,
      `${prefix}_event_1a`,
      route.eventLabels[0],
      route.actionLabels[2],
      `${prefix}_2`,
    );
    nodes[`${prefix}_event_1b`] = oneChoice(
      config.namespace,
      `${prefix}_event_1b`,
      route.eventLabels[1],
      route.actionLabels[3],
      `${prefix}_2`,
    );
    nodes[`${prefix}_2`] = {
      kind: "choice",
      ...
      choices: [
        {
          ...
          label: authored(
            ...
            `${prefix}_2`,
            "prepare",
            route.actionLabels[4],
          ),
          effects: [{ op: "increment", var: "preparation", by: 1 }],
          goto: `${prefix}_event_2`,
        },
        {
          id: `${prefix}_improvise`,
          label: authored(
            config.namespace,
            `${prefix}_2`,
            "improvise",
            route.actionLabels[5],
          ),
          effects: [{ op: "increment", var: "pressure", by: 1 }],
          goto: `${prefix}_event_2`,
        },
    ...
    nodes[`${prefix}_event_2`] = {
      ...
      text: authored(
        ...
        `${prefix}_event_2`,
        "text",
        "A seeded interruption arrives on schedule.",
      ),
      transitions: [
        {
          weight: 1,
          effects: [{ op: "increment", var: "connections", by: 1 }],
          goto: `${prefix}_event_2a`,
        },
        { weight: 1, goto: `${prefix}_event_2b` },
      ],
    };
    nodes[`${prefix}_event_2a`] = oneChoice(
      config.namespace,
      `${prefix}_event_2a`,
      route.eventLabels[2],
      route.actionLabels[6],
      `${prefix}_3`,
    );
    nodes[`${prefix}_event_2b`] = oneChoice(
      config.namespace,
      `${prefix}_event_2b`,
      route.eventLabels[3],
      route.actionLabels[7],
      `${prefix}_3`,
    );
    nodes[`${prefix}_3`] = {
      kind: "choice",
      ...
      choices: [
        ...
        },
        {
          ...
          requirementFail: authored(
            config.namespace,
            `${prefix}_3`,
            "favour_fail",
            "No one owes you that favour yet.",
          ),
          effects: [{ op: "decrement", var: "pressure", by: 1 }],
          goto: `${prefix}_4`,
        },
      ],
    };
      ...
      ...(ending.gate === "prepared"
        ? {
            ...
            requirementFail: authored(
              config.namespace,
              `${prefix}_4`,
              `ending_${ending.id}_fail`,
              "This ending needs more preparation.",
            ),
          }
        : {}),
    ...
    for (const suffix of [
      ...
      achievements.push({
        ...
        name: authored(
          ...
          `achievement_${nodeId}`,
          "name",
          `Found: ${route.memoryLabel}`,
        ),
        description: authored(
          config.namespace,
          `achievement_${nodeId}`,
          "description",
          "Discover an optional page on this route.",
        ),
        hidden: true,
        condition: {
    ...
    variables: {
      ...
      pressure: {
        ...
        label: authored(
          ...
          "var_pressure",
          "label",
          config.statLabels.pressure,
        ),
      },
      ...Object.fromEntries(
        config.routes.map((route) => [
          `remembered_${route.id}`,
          { type: "bool" as const, initial: false },
        ]),
      ),
    },
...
export function buildAdventureCampaign(
  config: AdventureConfig,
  ...
  return buildCampaign(campaign, [
    authored(config.namespace, "campaign", "title", config.title),
    ...authoredText,
  ]);
}
...
export function migrateV1AdventureState(
      ...
      errors: [
        {
          ...
          messageKey: "core.reason.migration_failed",
        },
      ],
      warnings: [],
    };
  ...
  const visitedCounts: Record<string, number> = Object.create(null) as Record<
    string,
    number
  >;
  ...
  if (
    routeDeclaration?.type === "enum" &&
    routeDeclaration.values !== undefined
  ) {
    const routeForCurrentNode = routeDeclaration.values.find((route) =>
      currentNodeId.startsWith(`${route}_`),
    );
    if (routeForCurrentNode !== undefined)
      variables.route = routeForCurrentNode;
  }
  const migrated: StoryGraphKindState = {
    ...
    variables,
    visitedCounts,
    ...(state.endingId === undefined
      ? {}
      : { endingId: endingMap[state.endingId] ?? state.endingId }),
  };
